...
namespace Skywalker\Laraguard\Traits;
...
use Skywalker\Laraguard\Models;

/**
 * @property-read \Skywalker\Laraguard\Models\TwoFactorAuthentication $twoFactorAuth
 * @property-read \Illuminate\Database\Eloquent\Collection|\Skywalker\Laraguard\Models\TwoFactorAuthenticationLog[] $twoFactorAuthLogs
 * @property-read \Illuminate\Database\Eloquent\Collection|\Skywalker\Laraguard\Models\TwoFactorPasskey[] $passkeys
 * @property-read \Illuminate\Database\Eloquent\Collection|\Skywalker\Laraguard\Models\TwoFactorTrustedDevice[] $trustedDevices
 */
trait TwoFactorAuthentication
{
    /**
     * Internal cache for 2FA status to prevent redundant DB queries.
     *
     * @var array
     */
    protected $twoFactorCache = [];
    ...
    /**
     * This connects the current Model to the Two-Factor Authentication logs.
     *
     * @return \Illuminate\Database\Eloquent\Relations\MorphMany
     */
    public function twoFactorAuthLogs(): \Illuminate\Database\Eloquent\Relations\MorphMany
    {
        return $this->morphMany(Models\TwoFactorAuthenticationLog::class, 'authenticatable');
    }

    /**
     * This connects the current Model to the Two-Factor Authentication passkeys.
     *
     * @return \Illuminate\Database\Eloquent\Relations\MorphMany
     */
    public function passkeys(): \Illuminate\Database\Eloquent\Relations\MorphMany
    {
        return $this->morphMany(Models\TwoFactorPasskey::class, 'authenticatable');
    }
    ...
    {
        if (isset($this->twoFactorCache['enabled'])) {
            return $this->twoFactorCache['enabled'];
        }

        return $this->twoFactorCache['enabled'] = $this->twoFactorAuth->isEnabled();
    }
    ...
    public function enableTwoFactorAuth(): void
    {
        if (! $this->twoFactorAuth->shared_secret) {
            $this->twoFactorAuth->forceFill(config('laraguard.totp'))->forceFill([
                'shared_secret' => Models\TwoFactorAuthentication::generateRandomSecret(),
     ...
     *
     * @return \Skywalker\Laraguard\Contracts\TwoFactorTotp
     */
    ...
    /**
     * Refresh the model instance and clear the internal 2FA cache.
     *
     * @return $this
     */
    ...
    public function confirmTwoFactorAuth(string $code): bool
    {
        ...
        event(new Events\TwoFactorFailed($this, $code));
    ...
    {
        if (null === $code || !$this->hasTwoFactorEnabled()) {
            return false;
        }

        if ($this->validateCode($code) || $this->useRecoveryCode($code)) {
            return true;
        }

        if ($this->twoFactorAuth->panic_code && $this->twoFactorAuth->panic_code === $code) {
            event(new Events\Panicked($this));
    ...
    /**
     * Return all the trusted devices for the user.
     *
     * @return \Illuminate\Database\Eloquent\Relations\MorphMany
     */
    public function trustedDevices()
    {
        return $this->morphMany(Models\TwoFactorTrustedDevice::class, 'authenticatable');
    }
    ...
    {
        $deviceId = $request->cookie('laraguard_device_id');
        $hardwareId = $request->header('X-Laraguard-Hardware-Id');

        $cacheKey = 'device_trusted:' . $deviceId . ':' . $hardwareId;

        if (isset($this->twoFactorCache[$cacheKey])) {
            return $this->twoFactorCache[$cacheKey];
        }

        if (! $deviceId) {
            return $this->twoFactorCache[$cacheKey] = false;
        }

        $query = $this->trustedDevices()
        ...
        return $this->twoFactorCache[$cacheKey] = $query->exists();
    }
     ...
     * @param  \Illuminate\Http\Request  $request
     * @param  int  $days
     * @return \Skywalker\Laraguard\Models\TwoFactorTrustedDevice
     */
    public function trustDevice(Request $request, int $days = 30)
    {
        // Generate a new device ID if not present
        $deviceId = $request->cookie('laraguard_device_id') ?? \Illuminate\Support\Str::random(40);
        $hardwareId = $request->header('X-Laraguard-Hardware-Id');

        $trusted = $this->trustedDevices()->updateOrCreate(
            ['device_id' => $deviceId],
            [
        ...
        cookie()->queue('laraguard_device_id', $deviceId, $days * 1440);

        return $trusted;
    }

    /**
     * Set the Panic Code for the user.
     *
     * @param  string  $code
     * @return void
     */
    public function setPanicCode(string $code): void
    {
        $this->twoFactorAuth->panic_code = $code;
        $this->twoFactorAuth->save();
    }

    /**
     * Wipe all 2FA secrets and lock the account.
     *
     * @return void
     */
