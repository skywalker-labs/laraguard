...
final class EnumMap implements Serializable, IteratorAggregate
{
    ...
    {
        return array_values(array_filter($this->keyUniverse, function ($key) {
            return null !== $this->values[$key->ordinal()];
        }));
    }

    /**
     * Filters the map using a predicate.
     *
     * @param callable $p function($value, $key)
     * @return self
     */
    public function filter(callable $p): self
    {
        $newMap = new self($this->keyType, $this->valueType, $this->allowNullValues);
    ...
    /**
     * Transforms values using a mapper.
     *
     * @param callable $f function($value, $key)
     * @return array
     */
    public function mapValues(callable $f): array
    {
        $result = [];
        foreach ($this->getIterator() as $key => $value) {
            $result[] = $f($value, $key);
    ...
    /**
     * Transforms keys using a mapper.
     *
     * @param callable $f function($key, $value)
     * @return array
     */
    public function mapKeys(callable $f): array
    {
        $result = [];
        foreach ($this->getIterator() as $key => $value) {
            $result[] = $f($key, $value);
    ...
    /**
     * Returns the first key in the map, or null if empty.
     */
    public function first(): ?AbstractEnum
    {
        foreach ($this->values as $ordinal => $value) {
            if (null !== $value) {
                return $this->keyUniverse[$ordinal];
            }
        }
        return null;
    }
    ...
    /**
     * Returns the last key in the map, or null if empty.
     */
    public function last(): ?AbstractEnum
    {
        $reversed = array_reverse($this->values, true);
        foreach ($reversed as $ordinal => $value) {
            if (null !== $value) {
                return $this->keyUniverse[$ordinal];
            }
        }
